...
function LoadData(){
    ...
    if(username!="" && password!=""){
      for(var i = 0; i < RaayegDAta1.length; i++)  {
      if(RaayegDAta1[i].usarname == username ){return}
      }
      localStorage.clear();
    }
  }
  ...
  function LoginUser()  {
    ...
    var password = document.getElementById("password").value;
      if(username!="" && password!=""){
      for(var i = 0; i < RaayegDAta1.length; i++)  {
      if(RaayegDAta1[i].usarname == username && 
        RaayegDAta1[i].password == password){
          if (typeof(Storage) !== "undefined") {
            localStorage.setItem("userRaayeg", username);
            localStorage.setItem("passRaayeg", password);
            localStorage.setItem("course1", RaayegDAta1[i].course);
          }
